...
#include "shader/ps.h"
// 影描画用
#include "shader/vs_shadow.h"  // モーションあり
...
void Shader::ShaderInit()
{
	...
	if (FAILED(manager->mpDevice.Get()->CreateBuffer(&cbDesc, NULL, mpConstantBufferWorld.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateConstantBufferWorld"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreateBuffer(&abDesc, NULL, mpConstantBufferAnime.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateConstantBufferAnime"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreateBuffer(&liDesc, NULL, mpConstantBufferLight.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateConstantBufferLight"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
...
void Shader::ShaderInitShadow()
{
	...
	//一番の親であるデバイス(ID3D11Device*型変数)の権利で、設定項目から頂点レイアウトを作成する
	//第1引数:設定値が格納されたバッファ　第2引数:設定値バッファのサイズ　第3引数:送り先となる頂点シェーダの名前　第4引数:送り先となる頂点シェーダのサイズ　第5引数:作成されたレイアウトの格納先
	if (FAILED(manager->mpDevice.Get()->CreateInputLayout(hInElementDesc, ARRAYSIZE(hInElementDesc), g_vs_shadow_main, sizeof(g_vs_shadow_main), mpShadowInputLayout.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowInputLayout"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}

	// モーションなし
	D3D11_INPUT_ELEMENT_DESC hInElementDescNm[] = { //D3D11_INPUT_ELEMENT_DESC : 頂点レイアウトを格納する構造体
			{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,    0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "DEPTH",    0, DXGI_FORMAT_R32G32B32_FLOAT,    0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	};
	//(ID3D11Device*型変数)->CreateInputLayout
	//一番の親であるデバイス(ID3D11Device*型変数)の権利で、設定項目から頂点レイアウトを作成する
	//第1引数:設定値が格納されたバッファ　第2引数:設定値バッファのサイズ　第3引数:送り先となる頂点シェーダの名前　第4引数:送り先となる頂点シェーダのサイズ　第5引数:作成されたレイアウトの格納先
	if (FAILED(manager->mpDevice.Get()->CreateInputLayout(hInElementDescNm, ARRAYSIZE(hInElementDescNm), g_vs2_shadow_main, sizeof(g_vs2_shadow_main), mpShadowInputLayoutNm.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowInputLayoutNm"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	//定数バッファ作成
	if (FAILED(manager->mpDevice.Get()->CreateBuffer(&cbDesc, NULL, mpShadowConstantBufferWorld.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowConstantBufferWorld"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	//定数バッファ作成
	if (FAILED(manager->mpDevice.Get()->CreateBuffer(&abDesc, NULL, mpShadowConstantBufferAnime.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowConstantBufferAnime"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	// モーションあり
	if (FAILED(manager->mpDevice.Get()->CreateVertexShader(&g_vs_shadow_main, sizeof(g_vs_shadow_main), NULL, mpShadowVertexShader.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowVertexShader"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	// モーションなし
	if (FAILED(manager->mpDevice.Get()->CreateVertexShader(&g_vs2_shadow_main, sizeof(g_vs2_shadow_main), NULL, mpShadowVertexShaderNm.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowVertexShaderNm"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreatePixelShader(&g_ps_shadow_main, sizeof(g_ps_shadow_main), NULL, mpShadowPixelShader.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateAhadowPixelShader"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreateSamplerState(&smpDesc, mpShadowSampler.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowSamplerState"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreateRasterizerState(&hRasterizerDesc, mpShadowRasterState.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowRasterizerState"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
...
void Shader::ShaderInitNoMotionShadow()
{
	...
	if (FAILED(manager->mpDevice.Get()->CreatePixelShader(&g_ps_shadow_main, sizeof(g_ps_shadow_main), NULL, mpShadowPixelShader.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowPixelShader"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
	...
	if (FAILED(manager->mpDevice.Get()->CreateSamplerState(&smpDesc, mpShadowSampler.GetAddressOf()))) {
		//作成に失敗したら警告を出してプログラムを強制終了
		MessageBox(NULL, TEXT("CreateShadowSampler"), TEXT("ERROR!!"), MB_ICONSTOP);
		PostQuitMessage(0);
	}
...
void ShaderInit(void) {
	...
	// 影
	gpShaderShadow = new Shader;
	gpShaderShadow->ShaderInitShadow();
