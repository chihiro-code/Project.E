...
// グローバル変数
//*************************************
bool gpSoundFlag[SOUND_LABEL_MAX];


// パラメータ構造体
typedef struct
{
	LPCWSTR filename;	// 音声ファイルまでのパスを設定
	bool bLoop;			// trueでループ。通常BGMはture、SEはfalse。
} PARAM;


// 音楽のフォーマットは「wav」でお願いします。
PARAM g_param[SOUND_LABEL_MAX] =
{
	//-------------------------------------------
	// BGM
	//-------------------------------------------
	{L"assets/bgm/battle01.wav", true},  // 戦闘　BGM（ループさせるのでtrue設定）


	//-------------------------------------------
	// SE
	//-------------------------------------------
	{L"assets/se/se_hit01.wav", false},	// ヒット（爆発）
...
//=============================================================================
// 更新処理（ここにサウンドの再生、停止などをまとめて書くのが理想！）← Game.cppで呼び出すだけで使える様に
//=============================================================================
void UpdateSound(void)
{
	// 試しに実装
	// PlaySoundを１回呼び出せば音楽が再生される、１回呼び出すだけで再生され続けます
	// Mキーで再生
	if (GetKeyboardPress(DIK_M)) {
		if (gpSoundFlag[BGM_BATTLE_01] == false) {
			PlaySound(BGM_BATTLE_01); // 再生
			gpSoundFlag[BGM_BATTLE_01] = true;
		}
	}
	// Nキーで停止
	if (GetKeyboardPress(DIK_N)) {
		if (gpSoundFlag[BGM_BATTLE_01] == true) {
			StopSound(BGM_BATTLE_01); // 停止
			gpSoundFlag[BGM_BATTLE_01] = false;
		}
	}
